...
public class DonationResourceImpl extends DonationResourceDecorator {
    ...
    public Donation createDonationViaFlipPaymentLink(VMJExchange vmjExchange) {
        ...
        String title = "Donasi untuk Program " + program.getNameProgram();
    ...
    public Donation createDonation(VMJExchange vmjExchange, UUID idDonation){
        ...
        UUID recordId = (((DonationDecorator) savedDonation).getRecord()).getIdDonation();
        Donation donation = record.createDonation(vmjExchange, recordId);
        Donation viaPaymentGateway = DonationFactory.createDonation("aisco.donation.viapaymentgateway.DonationImpl", idDonation, donation, title, status, vendorName, paymentLink, DonationImpl.class.getName());
    ...
    public Donation createDonation(VMJExchange vmjExchange, UUID idDonation, String objectName){
        ...
        UUID recordId = (((DonationDecorator) savedDonation).getRecord()).getIdDonation();
        Donation donation = record.createDonation(vmjExchange, recordId);
        Donation viaPaymentGateway = DonationFactory.createDonation("aisco.donation.viapaymentgateway.DonationImpl", idDonation, donation, title, status, vendorName, paymentLink, objectName);
    ...
    public void receiveCallback(VMJExchange vmjExchange) {
    	...
    	if (donationViaPaymentGateway.getStatus().equals("PENDING")) {
            if (status.equals("SUCCESSFUL")) {
            	...
            	//createIncome
            	String date = donation.getDateDonation();
            	long amount = donation.getAmountDonation();
            	String description = donation.getDescriptionDonation();
            	Program program = donation.getProgram();
            	String paymentMethod = donation.getPaymentMethodDonation();
