...
public class DonationResourceImpl extends DonationResourceComponent {
    ...
    public FinancialReport createIncome(VMJExchange vmjExchange) {
        ...
        String descriptionFinancialReport = (String) payload.get("descriptionDonation");
        String paymentMethodIncome= (String) payload.get("paymentMethodDonation");
        String idProgramStr = (String) payload.get("idProgram");
        String date = (String) payload.get("dateDonation");
        String amountStr = (String) payload.get("amountDonation");
		...
		FinancialReport financialReport = FinancialReportFactory.createFinancialReport("aisco.financialreport.core.FinancialReportImpl", date, amountFinancialReport, descriptionFinancialReport, program, coa);
		FinancialReport income = FinancialReportFactory.createFinancialReport("aisco.financialreport.income.FinancialReportImpl", financialReport, paymentMethodIncome);
		return income;
	}
    ...
    public Donation createDonation(VMJExchange vmjExchange, UUID idDonation, String objectName) {
        Map<String, Object> payload = vmjExchange.getPayload();
        String nameDonation = (String) payload.get("nameDonation");
        String email = (String) payload.get("email");
        String phone = (String) payload.get("phone");
        String descriptionDonation = (String) payload.get("descriptionDonation");
        String paymentMethodDonation = "";
        if (payload.get("paymentMethodDonation") != null) {
        	paymentMethodDonation = (String) payload.get("paymentMethodDonation");
        }
        String idProgramStr = (String) payload.get("idprogram");
        Program program = null;
        if (idProgramStr != null) {
...
//        String date = (String) payload.get("date");
        ...
        long amountDonation = 0L;
        if (payload.get("amountDonation") != null)  {
        	String amountStr = (String) payload.get("amountDonation");
        	amountDonation = Long.parseLong(amountStr);
        }
        if (amount < 0) {
            throw new FieldValidationException("Nilai amount harus lebih besar dari 0");
        }
        Donation donation = DonationFactory.createDonation("aisco.donation.core.DonationImpl", idDonation, nameDonation, email, phone,
                amountDonation, paymentMethodDonation, date, program, descriptionDonation, objectName);
...
//        String date = (String) payload.get("date");
     // set date as today
        ...
        Donation donation = DonationFactory.createDonation("aisco.donation.core.DonationImpl", idDonation, nameDonation, email, phone,
                amountDonation, paymentMethodDonation, date, program, descriptionDonation);
    ...
    public HashMap<String, Object> updateDonation(VMJExchange vmjExchange) {
        String idStr = (String) vmjExchange.getRequestBodyForm("idDonation");
        UUID idDonation = UUID.fromString(idStr);
        Donation donation = createDonation(vmjExchange, idDonation);
    ...
    public HashMap<String, Object> getDonation(VMJExchange vmjExchange) {
        String idStr = vmjExchange.getGETParam("idDonation");
        UUID idDonation = UUID.fromString(idStr);
